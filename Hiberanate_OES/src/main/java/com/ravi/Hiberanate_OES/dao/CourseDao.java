...
public class CourseDao {
    ...
    public static Course createCourse(Scanner sr, Session session) {
       // System.out.println("Course Table");
        ...
        // Fetch user without involving course
        ...
        if (user != null) {
            course.setUser(user);  // Associate the course with the user
            user.getCourse().add(course);  // Maintain bidirectional relationship
        } else {
    ...
    public static void deleteCourse(Scanner sr, Session session) {
    	System.out.println("Enter which courseId do you want to delete: ");
     	 int courseId = sr.nextInt();  // Example: Assume you want to delete the user with ID 123
     	 Course course = session.find(Course.class, courseId);  // Find the entity by its primary key
 ...
 public static void updateCourse(Scanner sr, Session session) {
    System.out.println("Enter Course id which you want to update: ");
       ...
       if(courseTable != null) // if the tables is present then enter into the loop and execute
       {
