...
public class ExamDao {
    ...
    public static Exam createExam(Scanner sr, Session session) {
     ...
     // Fetch course without involving user
        System.out.println("Enter Course Id for the Exam: ");
        int courseId = sr.nextInt();
        Course course = session.get(Course.class, courseId);  // Only access Course table

        if (course != null) {
            exam.setCourse(course);  // Associate the exam with the course
            course.getExam().add(exam);  // Maintain bidirectional relationship
        } else {
    ...
    public static void deleteExam(Scanner sr, Session session) {
	    	System.out.println("Enter which examId do you want to delete: ");
		       int examId = sr.nextInt();  // Example: Assume you want to delete the user with ID 123
		       Exam exam = session.find(Exam.class, examId);  // Find the entity by its primary key
    ...
    public static void updateExam(Scanner sr, Session session) {
    	...
    	if(examTable != null) // if the tables is present then enter into the loop and execute
    	{
